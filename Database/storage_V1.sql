...
CREATE TABLE WAREHOUSE_DETAIL (
	...
	CONSTRAINT PK_WAREHOUSE_DETAIL PRIMARY KEY (WAREHOUSE_ID, ITEM_ID, MONTH),
...
SELECT *FROM IMPORT_ITEM_DETAIL
	...
	SELECT INVENTORY_INFO.ITEM_ID, INVENTORY_INFO.INVENTORY, IMPORT_EXPORT_TABLE.SUM_QTY_IMPORT, 
		IMPORT_EXPORT_TABLE.SUM_PRICE_IMPORT, IMPORT_EXPORT_TABLE.SUM_QTY_EXPORT,  
		(INVENTORY_INFO.INVENTORY + IMPORT_EXPORT_TABLE.SUM_QTY_IMPORT - IMPORT_EXPORT_TABLE.SUM_QTY_EXPORT) AS LAST_INVENTORY
		FROM (SELECT ITEM_ID, SUM(QUANTITY) AS INVENTORY 
				FROM WAREHOUSE_DETAIL
				WHERE MONTH = 11
				GROUP BY ITEM_ID) INVENTORY_INFO
				LEFT JOIN
				(SELECT IMPORT_INFO.ITEM_ID, SUM_QTY_IMPORT, SUM_PRICE_IMPORT, SUM_QTY_EXPORT FROM 
					((SELECT ITEM_ID, SUM(QTY) AS SUM_QTY_IMPORT, SUM(PRICE) AS SUM_PRICE_IMPORT 
						FROM IMPORT_ITEM_DETAIL 
						WHERE IMPORT_ITEM_ID IN (SELECT ID FROM IMPORT_ITEM WHERE MONTH(CREATED) = 12)
						GROUP BY ITEM_ID) IMPORT_INFO LEFT JOIN
					 (SELECT ITEM_ID, SUM(QTY) AS SUM_QTY_EXPORT 
						FROM EXPORT_ITEM_DETAIL 
						WHERE EXPORT_ITEM_ID IN (SELECT ID FROM EXPORT_ITEM WHERE MONTH(CREATED) = 12)
						GROUP BY ITEM_ID) EXPORT_INFO ON IMPORT_INFO.ITEM_ID = EXPORT_INFO.ITEM_ID)) IMPORT_EXPORT_TABLE ON INVENTORY_INFO.ITEM_ID = IMPORT_EXPORT_TABLE.ITEM_ID
...
SELECT ITEM.ID, ITEM.CODE, ITEM.NAME, UNIT.NAME, WAREHOUSE_INFO.INVENTORY, WAREHOUSE_INFO.SUM_QTY_IMPORT, 
		WAREHOUSE_INFO.SUM_PRICE_IMPORT, WAREHOUSE_INFO.SUM_QTY_EXPORT, WAREHOUSE_INFO.LAST_INVENTORY,
		GROUPS.NAME, SUPPLIER.NAME_SUPPIER
FROM (SELECT INVENTORY_INFO.ITEM_ID, INVENTORY_INFO.INVENTORY, IMPORT_EXPORT_TABLE.SUM_QTY_IMPORT, 
		IMPORT_EXPORT_TABLE.SUM_PRICE_IMPORT, IMPORT_EXPORT_TABLE.SUM_QTY_EXPORT,  
		(INVENTORY_INFO.INVENTORY + IMPORT_EXPORT_TABLE.SUM_QTY_IMPORT - IMPORT_EXPORT_TABLE.SUM_QTY_EXPORT) AS LAST_INVENTORY
		FROM (SELECT ITEM_ID, SUM(QUANTITY) AS INVENTORY 
				FROM WAREHOUSE_DETAIL
				...
				(SELECT IMPORT_INFO.ITEM_ID, SUM_QTY_IMPORT, SUM_PRICE_IMPORT, SUM_QTY_EXPORT FROM 
					((SELECT ITEM_ID, SUM(QTY) AS SUM_QTY_IMPORT, SUM(PRICE) AS SUM_PRICE_IMPORT 
					 ...
					 (SELECT ITEM_ID, SUM(QTY) AS SUM_QTY_EXPORT 
						FROM EXPORT_ITEM_DETAIL 
						WHERE EXPORT_ITEM_ID IN (SELECT ID FROM EXPORT_ITEM WHERE MONTH(CREATED) = 12)
						GROUP BY ITEM_ID) EXPORT_INFO ON IMPORT_INFO.ITEM_ID = EXPORT_INFO.ITEM_ID)) IMPORT_EXPORT_TABLE ON INVENTORY_INFO.ITEM_ID = IMPORT_EXPORT_TABLE.ITEM_ID) WAREHOUSE_INFO,
		ITEM, SUPPLIER, UNIT, GROUPS
WHERE WAREHOUSE_INFO.ITEM_ID = ITEM.ID
	AND ITEM.SUPPLIER_ID = SUPPLIER.ID
	AND ITEM.UNIT_ID = UNIT.ID
